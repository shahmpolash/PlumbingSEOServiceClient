...
const AddProductVariation = () => {
  ...
  useEffect(() => {
    fetch(`https://plumbingseoservice-e14c7ccb325f.herokuapp.com/product-variation/${id}`)
      .then((res) => res.json())
  ...
  const handleVariation = (event) => {
    ...
    const url = `https://plumbingseoservice-e14c7ccb325f.herokuapp.com/add-variation/${variation._id}`;
    fetch(url, {
      method: 'PUT',
      headers: {
